...
import { FaFilePdf, FaCheck, FaFileArchive, FaComments } from 'react-icons/fa';
          ...
          <div key={challenge.id} className="bg-white p-6 rounded-lg shadow-md">
            <div className="flex flex-col sm:flex-row gap-4 items-start">
              ...
              <div className="flex-grow w-full">
                  ...
                  {['IN_PROGRESS', 'COMPLETED', 'DONE'].includes(challenge.status) && (<div className="mt-4"><h4 className="font-semibold text-slate-600 border-b pb-2 mb-2">Hasil Pekerjaan ({challenge.submissions?.length || 0})</h4>{challenge.submissions?.length > 0 ? (<ul className="space-y-3 max-h-48 overflow-y-auto">{challenge.submissions.map(s => <li key={s.id} className="bg-slate-50 p-3 rounded-md border"><div className="flex justify-between items-start"><div><p className="font-semibold">{s.author.name}</p><p className="text-sm text-gray-600 mt-1">"{s.notes || 'Tidak ada catatan.'}"</p><a href={s.fileUrl} target="_blank" rel="noopener noreferrer" className="text-indigo-600 hover:underline text-sm mt-2 inline-flex items-center gap-1"><FaFileArchive /> Lihat File .GLB</a></div><div className="flex items-center gap-2">{s.status === 'PENDING' ? (<><button onClick={() => handleReview(s.id, 'REVISION_REQUESTED')} className="bg-yellow-500 text-white text-xs font-bold py-1 px-2 rounded-md hover:bg-yellow-600">Revisi</button><button onClick={() => handleReview(s.id, 'APPROVED')} className="bg-green-500 text-white text-xs font-bold py-1 px-2 rounded-md hover:bg-green-600">Setujui</button></>) : (<span className={`text-sm font-bold ${s.status === 'APPROVED' ? 'text-green-600' : 'text-yellow-600'}`}>{s.status}</span>)}</div></div></li>)}</ul>) : <p className="text-sm text-gray-500">Pekerja belum mengumpulkan hasil.</p>}</div>)}
              </div>
            </div>
          </div>
        ))}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {myProposals.map(proposal => (
                <div key={proposal.id} className="bg-white rounded-lg shadow-lg flex flex-col">
                    {/* PERBAIKAN: onClick sekarang tidak melakukan apa-apa */}
                    ...
                    <div className="p-3 border-t bg-slate-50 flex justify-between items-center">
                        ...
                        {/* PERBAIKAN: Tombol chat hanya muncul jika proposal disetujui */}
