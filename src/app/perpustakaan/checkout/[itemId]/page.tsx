...
const CheckoutPage = () => {
      ...
      <div className={`min-h-screen transition-colors duration-300 ${theme === 'light' ? 'bg-gray-50' : 'bg-gray-900'
        ...
        <div className="pt-28 flex items-center justify-center">
          <div className={`p-8 rounded-xl ${theme === 'light' ? 'bg-white shadow-lg' : 'bg-gray-800 shadow-xl'
            ...
            <p className={`text-lg ${theme === 'light' ? 'text-gray-700' : 'text-gray-200'
              }`}>Memuat...</p>
          </div>
        </div>
        <Footer />
      ...
      <div className={`min-h-screen transition-colors duration-300 ${theme === 'light' ? 'bg-gray-50' : 'bg-gray-900'
        ...
        <div className="pt-28 flex items-center justify-center">
          <div className={`p-8 rounded-xl ${theme === 'light' ? 'bg-white shadow-lg' : 'bg-gray-800 shadow-xl'
            ...
            <p className={`text-lg mb-4 ${theme === 'light' ? 'text-gray-700' : 'text-gray-200'
              }`}>Item tidak ditemukan.</p>
            <Link
              className={`inline-block px-6 py-2 rounded-lg font-semibold transition-colors ${theme === 'light'
  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 ${theme === 'light' ? 'bg-gray-50 text-gray-900' : 'bg-gray-900 text-gray-50'
      }`}>
      <Header />
      <div className="pt-28 pb-20 px-6">
        <div className="max-w-2xl mx-auto">
          <div className={`p-8 rounded-2xl shadow-xl ${theme === 'light' ? 'bg-white border border-gray-200' : 'bg-gray-800 border border-gray-700'
            }`}>
            <div className="text-center mb-8">
              <h1 className={`font-display text-3xl font-bold mb-2 ${theme === 'light' ? 'text-gray-900' : 'text-white'
                }`}>Checkout</h1>
              <div className={`w-20 h-1 mx-auto rounded-full ${theme === 'light' ? 'bg-blue-600' : 'bg-lime-400'
                }`}></div>
            </div>

            {/* Item Info */}
            <div className={`p-6 rounded-xl mb-6 ${theme === 'light' ? 'bg-gray-50 border border-gray-200' : 'bg-gray-700/50 border border-gray-600'
              }`}>
              <h2 className={`text-lg font-semibold mb-2 ${theme === 'light' ? 'text-gray-800' : 'text-gray-200'
                }`}>Item:</h2>
              ...
              <div className="flex items-center justify-between">
                ...
                <span className={`text-2xl font-bold ${theme === 'light' ? 'text-blue-600' : 'text-lime-400'
                  }`}>Rp {priceLabel}</span>
              </div>
            </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                {(['DANA', 'OVO', 'GoPay', 'PayPal'] as Method[]).map(m => (
                  <button
                    key={m}
                    onClick={() => setMethod(m)}
                    className={`p-4 rounded-xl font-semibold transition-all duration-200 ${method === m
                        ? theme === 'light'
            ...
            {/* Error Message */}
            {error && (
              <div className={`p-4 rounded-lg mb-6 ${theme === 'light' ? 'bg-red-50 border border-red-200' : 'bg-red-900/20 border border-red-700'
                }`}>
                <p className={`text-sm font-medium ${theme === 'light' ? 'text-red-800' : 'text-red-400'
                  }`}>{error}</p>
              </div>
            )}
            ...
            <button
              ...
              disabled={processing}
              type="button"
              className={`w-full py-4 px-6 rounded-xl font-bold text-lg transition-all duration-200 ${processing
            ...
            <p className={`text-xs text-center mt-4 ${theme === 'light' ? 'text-gray-500' : 'text-gray-400'
              ...
              <FaExclamationTriangle className="inline mr-1 mb-0.5" /> Simulasi pembayaran dummy untuk testing
            </p>
          </div>
        </div>
      </div>
